...
import { updateTicketStatusApi } from '../../services/api';
import {
  X,
  CheckCircle2,
  Send,
...
export const ActionDrawer: React.FC<ActionDrawerProps> = ({
  ...
  const [status, setStatus] = useState<GrievanceStatus>('Pending');
  const [department, setDepartment] = useState<DepartmentType>('Sanitation & Waste');
  ...
  useEffect(() => {
    if (grievance) {
      ...
      setOfficer(grievance.Assigned_Officer || '');
      setEscalationNote('');
      setIsSaved(false);
      setSyncNotice(null);
    }
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    // Attempt PATCH to FastAPI backend /api/tickets/{ticket_id}
    try {
      await updateTicketStatusApi(grievance.Complaint_ID, status);
      setSyncNotice('Synced to FastAPI backend (PATCH /api/tickets)');
    } catch (err: unknown) {
      console.warn(`Backend PATCH /api/tickets/${grievance.Complaint_ID} failed, updating locally:`, err);
      setSyncNotice('Updated in local session memory');
    }
            ...
            <form onSubmit={handleSave} className="space-y-5 pt-2">
              ...
              <div>
                <label className="block text-xs font-semibold uppercase tracking-wider mb-2 text-slate-300">
                  Update Grievance Status (PATCH /api/tickets)
                </label>
                <div className="grid grid-cols-2 gap-2">
                  {(['Pending', 'In Progress', 'Resolved', 'Escalated'] as GrievanceStatus[]).map((st) => (
                    <button
                      ...
                      className={`p-2.5 rounded-xl text-xs font-bold border transition flex items-center justify-center space-x-1.5 cursor-pointer ${
                        status === st
              ...
              <div className="pt-4 border-t border-slate-800 flex items-center justify-end space-x-3">
                ...
                <button
                  type="submit"
                  disabled={isSaving}
                  className="px-5 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold transition shadow-lg shadow-emerald-600/30 flex items-center space-x-1.5 cursor-pointer disabled:opacity-50"
                >
                  {isSaving ? (
                    <>
                      <Activity className="w-4 h-4 animate-spin text-white" />
                      <span>Saving to FastAPI...</span>
                    </>
                  ) : isSaved ? (
                    <>
